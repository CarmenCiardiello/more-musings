...
```
This table contains batter rate stats for all pitches faced and is segmented by pitch type. To only look at all pitches faced, type "All" into the dialog box in the type column. I recommend filtering with some minimum for number of pitches faced. All columns are sortable. 
```{r}
#| echo: false
#| warning: false
...
hitting_table %>% 
  ...
  add_title(
    title = "For filtering by number of pitches, type in the minimum pitches to display", 
    font_size = 16
    )
